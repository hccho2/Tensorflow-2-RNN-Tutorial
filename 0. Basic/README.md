...
- 예를 들어, 단어를 몇 차원 vector로 만들 것인가를 미리 정해야 하는데, 이를 embedding dimension이라 하는데, hyperparameter로 미리 정해주어야 한다. 이것이 위에서 언급한 input_dim이다.
- 단어를 Embedding하는 것 자체를 목적으로 하는 모델도 있다. Skip-Gram, CBOW같은 것들이다. 이런 모델로 통해서 잘 만들어진 Embedding이 있다면 불러와서 사용할 수도 있다.
